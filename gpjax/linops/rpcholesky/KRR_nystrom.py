...
class KRR_Nystrom:
    ...
    def predict(self, Xts):
        ts = time.time()
        K_pred = NonsymmetricKernelMatrix(
            Xts, self.Xtr, kernel=self.kernel, bandwidth=self.bandwidth
        )
        K_pred_exact = K_pred[:, :]
    ...
    def fit_Nystrom(
        self, Xtr, Ytr, lamb, sample_num, sample_method, solve_method="Direct", ite=400
    ):
        self.Xtr = Xtr
        self.K = KernelMatrix(Xtr, kernel=self.kernel, bandwidth=self.bandwidth)
        ts = time.time()
