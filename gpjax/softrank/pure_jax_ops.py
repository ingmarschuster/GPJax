...
from .third_party import pure_jax_isotonic as pji
...
if True:
    ...
    def isotonic_l2(input_s, input_w=None):
        ...
        with jax.disable_jit(True):
            solution = pji.isotonic_l2(input_s - input_w)
        return solution
    ...
    def isotonic_kl(input_s, input_w=None):
        ...
        input_w = input_w.astype(input_s.dtype)
        with jax.disable_jit(True):
            solution = pji.isotonic_kl(input_s, input_w.astype(input_s.dtype))
        return solution
    ...
    def isotonic_l2(input_s, input_w=None):
        ...
        with jax.disable_jit(True):
            solution = onp.zeros_like(input_s)
            isotonic.isotonic_l2(onp.array(input_s - input_w), solution)
    ...
    def isotonic_kl(input_s, input_w=None):
        ...
        input_w = input_w.astype(input_s.dtype)
        with jax.disable_jit(True):
            solution = onp.zeros(len(input_s)).astype(input_s.dtype)
            isotonic.isotonic_kl(
                onp.array(input_s), input_w.astype(input_s.dtype), solution
            )
            solution = np.array(solution)
        return solution
...
def _inv_permutation(permutation):
    ...
    inv_permutation = inv_permutation.at[permutation].set(np.arange(len(permutation)))
