...
@dataclass
class ReduceEncoder(gpx.rkhs.base.AbstractEncoder):
    ...
    k: gpx.kernels.AbstractKernel = param_field(gpx.kernels.RBF())
    red: gpx.rkhs.base.AbstractReduce = param_field(gpx.rkhs.reduce.NoReduce())

    def __call__(self, inp: Any) -> gpx.rkhs.vector.RkhsVec:
        """Encodes input array into an RKHS vector, where the kernel is applied to each element of the input array.
        ...
        """
        return self.red @ gpx.rkhs.vector.RkhsVec(k=self.k, insp_pts=inp)
...
D = gpx.Dataset(X=ds.Dataset.from_dict({"x": x}), y=y)
...
# %%
compl_reduce = gpx.rkhs.reduce.BalancedRed(
    points_per_split=2, average=True
) @ gpx.rkhs.reduce.TileView(tile_times=2)

lin_red = gpx.rkhs.reduce.LinearReduce(jnp.ones((1, 1)) * 20).replace_trainable(
    linear_map=False
)

comp_red2 = gpx.rkhs.reduce.BalancedRed(points_per_split=2, average=True).linearize(
    (2, 1)
) @ gpx.rkhs.reduce.TileView(tile_times=2).linearize((1, 1)).replace_trainable(
    linear_map=False
)

comp_red3 = gpx.rkhs.reduce.LinearReduce(
    jnp.ones((1, 2)) / 2
) @ gpx.rkhs.reduce.LinearReduce(
    linear_map=jnp.array([[1.0], [1.0]], dtype=jnp.float64)
)
kernel = gpx.kernels.EncoderKernel(enc=ReduceEncoder(red=comp_red2))
kernel = gpx.kernels.EncoderKernel(
    enc=gpx.rkhs.encoder.HfStandardEncoder(),
)
...
# %%
prior_dist = prior.predict(ds.Dataset.from_dict({"x": xtest}))
...
latent_dist = opt_posterior.predict(
    ds.Dataset.from_dict({"x": xtest + 2}), train_data=D
)
