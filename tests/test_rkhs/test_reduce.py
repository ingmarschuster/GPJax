...
def test_MovingAverage():
    ...
    for r in (
        MovingAverage(2, 4, False, "linear"),
        MovingAverage(2, 4, False, "sparse"),
        MovingAverage(2, 4, False, "binary"),
    ):
        assert np.allclose(r @ gram, expected_result)

    idx_res = np.array([gram[i].sum(0) for i in MovingAverage(2, 4, False, "index")])
    assert np.allclose(idx_res, expected_result)

    expected_result = expected_result / 2
    for r in (
        MovingAverage(2, 4, True, "linear"),
        MovingAverage(2, 4, True, "sparse"),
        MovingAverage(2, 4, True, "binary"),
    ):
        assert np.allclose(r @ gram, expected_result)
